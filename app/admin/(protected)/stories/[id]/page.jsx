// app/admin/(protected)/stories/[id]/page.jsx
// Edit an existing Tale directly by story id — whether it came from
// an approved submission or was written from scratch. No submission
// row involved, so SubmissionEditor automatically hides Reject.
...
function reshapeStoryForEditor(story) {
  ...
  return {
    ...
    story_type: story.story_type ?? 'tale',
    story: story.content ?? '',
    ...
    <div>
      <header style={{ padding: '1.5rem 6vw 0' }}>
        <p style={{ fontFamily: 'var(--body)', color: 'var(--ink)', fontSize: '0.95rem', margin: '0 0 1rem' }}>
          Editing story: <strong>{story.title}</strong> · Status: <strong>{story.status}</strong>
        </p>
            ...
            {!story.claimed_by && (
              <>
                <p style={{ fontFamily: 'var(--body)', fontSize: '0.85rem', color: 'var(--grey)', margin: '0 0 0.5rem' }}>
                  Claim link for {story.author_name || 'the author'}:
                </p>
                <code
                  style={{
                    display: 'block',
