// app/admin/(protected)/submissions/[id]/page.jsx
    ...
    <div>
      <header style={{ padding: '1.5rem 6vw 0' }}>
        <p style={{ fontFamily: 'var(--body)', color: 'var(--grey)', fontSize: '0.9rem', margin: '0 0 0.3rem' }}>
          Reviewing submission from {submission.name || 'Unknown'}
        ...
        <p style={{ fontFamily: 'var(--body)', color: 'var(--ink)', fontSize: '0.95rem', margin: '0 0 1rem' }}>
          ...
          {existingStory && (
            <>
              {' '}
              · Draft story exists (last saved slug: <strong>{existingStory.slug || '—'}</strong>)
            </>
          )}
        </p>
            ...
            {!story.claimed_by && (
              <>
                <p style={{ fontFamily: 'var(--body)', fontSize: '0.85rem', color: 'var(--grey)', margin: '0 0 0.5rem' }}>
                  Send this link to {submission.name || 'the author'} so they can claim their story:
                </p>
                <code
                  style={{
                    display: 'block',
