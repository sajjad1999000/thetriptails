...
export function getStoriesByAuthor(authorSlug) {
  return stories.filter((s) => s.author.slug === authorSlug);
}

export function getAuthorBySlug(authorSlug) {
  const story = stories.find((s) => s.author.slug === authorSlug);
  return story ? story.author : null;
}

export function getAllAuthorSlugs() {
  return [...new Set(stories.map((s) => s.author.slug))];
}
